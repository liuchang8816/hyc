...
// 文件名称 ：WorkFlow3DRaster.cs 
// ================================= 
// 创 建 者 ：yangkang 
// 创建日期 ：2021/4/23 18:59:24 
// 功能描述 ： 
// 使用说明 ： 
//
//
...
namespace RasterDetectionMod.Classes
{
    ...
    /// 3D光栅检测流程
    /// </summary>
    public class WorkFlow3DRaster : WorkFlow
    {
        /// <summary>
        /// 检测结果
        /// </summary>
        ...
        public override void Grab()
        {
            ...
            bitmap.Save(dateInfo + "\\Img3D\\Picture" + PhotoNum + ".jpg");
            bitmapdepth.Save(dateInfo + "\\Img3D\\depthPicture" + PhotoNum + ".jpg");
            FormMain.Instance.ShowPicture(bitmap, bitmapdepth);
        ...
        public override void CheckPhoto()
        {
            ...
            iParams[0] = ImgWidth;
            iParams[1] = ImgHeight;
            iParams[2] = PhotoNum - 1;
            iParams[3] = SzResult.Length;
            float[] fParams = new float[3];
            fParams[0] = Camera3D.ThreArea;
            FormMain.Instance.AppendTxt3DLog($"3D光栅检测开始!", Color.Black);
            int ret = Cam3DRaster.GetDetectResult(strTXTDir, StrMarkDir, strMarkPath, strCaliPath, strOutDir, iParams, fParams, SzResult);
            FormMain.Instance.AppendTxt3DLog($"3D光栅检测返回值{ret}!", Color.Black);
            if (ret == 0)
            {
                ...
                defectList.ConvertErrorInfo(errorInfo, OffsetX, OffsetY);
            ...
            {
                FormMain.Instance.AppendTxt3DLog("3D光栅检测失败!", Color.Red);
            }
        ...
        private void GenerateIndexImage(DefectList3D total)
        {
            try
            {
                Bitmap bitmap = new Bitmap(2000, 1500);
                using (Graphics g = Graphics.FromImage(bitmap))
                {
                    ...
                    foreach (var layer in total.ListLayers)
                    {
                        ...
                        {
                            //X,Y毫米
                            int x = (int)(defect.WorldX * 50);
                            int y = (int)(defect.WorldY * 50);
                            //W,H:微米  
                ...
                using (Graphics g = Graphics.FromImage(bitmap))
                {
                    ...
                    g.DrawString("层一", new Font("微软雅黑", 10.5f), Brushes.Gray, new Point(35, 10));
                    g.FillRectangle(Brushes.Yellow, new Rectangle(10, 35, 20, 20));
                    g.DrawString("层二", new Font("微软雅黑", 10.5f), Brushes.Gray, new Point(35, 35));
                    g.FillRectangle(Brushes.Blue, new Rectangle(10, 60, 20, 20));
                    g.DrawString("层三", new Font("微软雅黑", 10.5f), Brushes.Gray, new Point(35, 60));
                    g.FillRectangle(Brushes.Gray, new Rectangle(10, 85, 20, 20));
                    g.DrawString("层四", new Font("微软雅黑", 10.5f), Brushes.Gray, new Point(35, 85));
                }
